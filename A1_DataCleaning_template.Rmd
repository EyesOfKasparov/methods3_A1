...
```{r}
demo <-
  read_csv("https://www.dropbox.com/s/lol8f5m4pgrpmto/demo_train.csv?dl=1") %>%
  rename(VISIT = Visit) %>%
  rename(ID = Child.ID) %>%                 
  mutate(ID = str_remove_all(ID, "\\."))    # Remove .'s from ID

utt <- 
  read_csv("https://www.dropbox.com/s/usyauqm37a76of6/LU_train.csv?dl=1") %>%
  rename(ID = SUBJ) %>%
  mutate(VISIT = parse_number(VISIT)) %>%                 
  mutate(ID = str_remove_all(ID, "\\."))

words <- 
  read_csv("https://www.dropbox.com/s/8ng1civpl2aux58/token_train.csv?dl=1") %>%
  rename(ID = SUBJ) %>%
  mutate(VISIT = parse_number(VISIT)) %>%                 
  mutate(ID = str_remove_all(ID, "\\."))
...
```{r}
df <- left_join(demo, words, by = c("ID", "VISIT")) %>%
  left_join(utt, by = c("ID", "VISIT")) %>%
  select(c(ID, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization, MOT_MLU, CHI_MLU,types_MOT,types_CHI,tokens_MOT,tokens_CHI))
```
...
```{r}
df2 <- filter(df, VISIT == 1) %>%
  select(c(ID, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)) %>%
  rename(ADOS1 = ADOS) %>%
...
dfa <- read_csv("anonymized_data.csv")
...
dfa %>%
  ...
  select(c(ID, CHI_MLU, MOT_MLU, Diagnosis))

# 2
dfa %>%
  select(c(ID, ID, CHI_MLU, CHI_MLU, Diagnosis))

# Then it is only represented once which is quite practical
...
dfa %>%
  group_by(ID) %>%
  summarise(CHI_MLU_M = mean(CHI_MLU)) %>%
  right_join(dfa, by = c("ID"))

# 3
# We can create a mixed effects linear model that has different intercepts and slopes based on the child - pretty cool if we want some nice models
# The other would be a t-test which is pretty boring and normal
lmerTest::lmer(CHI_MLU ~ Socialization * Gender + Age + (VISIT | ID), dfa)
